...
curdata <- curdata %>% mutate(nr= if_else(answer == 3, nr/2, as.double(nr)))
curdata <- curdata %>% mutate(question = case_match(question, 
                                                    "QU05_01" ~ "(3A) The tool emphasizes what is special or unique in the data.",
                                                    "QU05_02" ~ "(3B) The tool distinguishes positive and negative contributions of features.",
                                                    "QU05_03" ~ "(3C) The tool offers an easily interpretable summary of the features",
                                                    "QU05_04" ~ "(3D) The tool helps me validate if I can trust the shown results.",
                                                    "QU06_04" ~ "(4A) The charts provided by the tool are easy to interpret.",
                                                    "QU06_05" ~ "(4B) The charts provided by the tool are visually pleasing.",
                                                    "QU06_06" ~ "(4C) The charts provided by the tool are helpful for finding feature interactions.",
                                                    "QU06_07" ~ "(4D)  The charts provided by the tool are helpful for interpreting feature interactions."))
...
ggplot(curdata, aes(x="", y=nr, fill=answer)) + 
 ...
 # geom_label(data=curdata %>% filter(answer %in% c(3,4,5)), aes(y=nr*1.5, label=nr))+
  ...
  theme(legend.justification ="center")
  ggsave("images/ESG.jpg", height=7, width=8)
