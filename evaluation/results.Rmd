...
curdata <- curdata %>% mutate(nr= if_else(answer == 3, nr/2, as.double(nr)))
curdata <- curdata %>% mutate(question = case_match(question, 
                                                    "QU04_01" ~ "I am confident in this tool. I feel that it works well.",
                                                    "QU04_02" ~ "The outputs of this tool are very predictable.",
                                                    "QU04_03" ~ "This tool is very reliable. I can count on it to be correct all the time.",
                                                    "QU04_04" ~ "I feel safe that when I rely on this tool I will get the right answers.",
                                                    "QU04_05" ~ "This tool is efficient in that it works very quickly.",
                                                    "QU04_06" ~ "This tool can perform the task better than a novice human user.",
                                                    "QU04_07" ~ "I like using this tool for decision making."))
...
ggplot(curdata, aes(x="", y=nr, fill=answer)) + 
 ...
 # geom_label(data=curdata %>% filter(answer %in% c(3,4,5)), aes(y=nr*1.5, label=nr))+
  ...
  ggsave("images/TXAI.jpg", height=6, width=8)
...
ggplot(curdata, aes(x="", y=nr, fill=answer)) + 
 ...
 # geom_label(data=curdata %>% filter(answer %in% c(3,4,5)), aes(y=nr*1.5, label=nr))+
  ...
  theme(legend.justification ="center")
  ggsave("images/Custom.jpg", height=8.5, width=8)
